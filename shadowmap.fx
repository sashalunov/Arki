...
float4   g_vLightAmbient = float4( 0.0f, 0.3f, 0.3f, 1.0f );  // Use an ambient light of 0.3
                 ...
                 float4 vPosLight : TEXCOORD3 ) : COLOR
{
    ...
    if( dot( vLight, g_vLightDir ) > g_fCosTheta ) // Light must face the pixel (within Theta)
    {
		...
		float spotDot = dot(vLight, normalize(g_vLightDir));
		float spotEffect = smoothstep(g_fCosTheta, 0.96, spotDot);
        ...
        // Light it
        Diffuse = ( saturate( dot( -vLight, normalize( vNormal ) ) ) * spotEffect * LightAmount * g_vLightDiffuse*  ( 1 - g_vLightAmbient ) + g_vLightAmbient )* g_vMaterial  ;
